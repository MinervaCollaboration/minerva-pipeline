...
#            print "WLS pars: ", par[1:4]
    ...
    def lnprior(self, par, tel):
        """
        Log(Prior), natural log of priors, for emcee_fitter
        ...
        cz_cond = (par[0] < (self.par[0 + tel*self.parspertrace] + 1.1e-4)) and \
                  (par[0] > (self.par[0 + tel*self.parspertrace] - 1.1e-4))

        # w0: +/-1 A range for wavelength zero point
        w0_cond = (par[1] < (self.par[1 + tel*self.parspertrace] + 1.0)) and \
                  (par[1] > (self.par[1 + tel*self.parspertrace] - 1.0))

        # dwdx: some realistic range for MINERVA, normally it's between 0.016 and 0.030
        dw_cond = (0.01 <= par[2] <= 0.04)

        # IP
        ip_ind = [5,6]  # easier for future if there are more IP parameters
        ip_cond = [(-1.0 <= par[item] <= 1.0) for item in ip_ind]

        if cz_cond and w0_cond and dw_cond and all(ip_cond):
            return 0.0
        return -np.inf
    ...
    def lnlikelihood(self, par, tel):
        ...
        resid = self.obchunk[tel, :][0] - current_model

        return -0.5 * np.sum(resid**2 * self.obchunk[tel, :][0])
    ...
    def emcee_fitter(self, nwalkers, niter, nburn):
        ...
        for tel, is_active in enumerate(self.active):
            if is_active:
                ...
                this_bpar = sampler.flatchain[m,:]  # mode of posterior distribution, "best parameters"
