...
#            obchunk[i,:] = crclean(obspec[i,:]) # remove hot/dead pixels
        ...
        par[6 + N*parspertrace] = IP broadening for telescope N
        par[7 + N*parspertrace] = IP skew for telescope N
        """
        self.parspertrace = 8
        ...
        for i in range(spectrum.ntel):
            ...
            # IP broadening guess
            self.initpar[6+tel*self.parspertrace] = 0.2
            ...
            # IP skew guess
            self.initpar[7+tel*self.parspertrace] = 0.0
            self.parinfo[7+tel*self.parspertrace]['limited'] = (True,True)
            self.parinfo[7+tel*self.parspertrace]['limits'] = (-1.0, 1.0)
            if fixip: self.parinfo[7+tel*self.parspertrace]['fixed'] = True
    ...
    for i, Ord in enumerate(order):
        for j, Pix in enumerate(pixel):
            ...
            for ii in range(ch.ntel):
                ...
                exec("chrec[chind].alpha%s  = bp[7 + tel*parspertrace]" % (ii+1))
