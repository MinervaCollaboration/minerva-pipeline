...
class FourChunk(object):
    def __init__(self, obsname, order, lpix, dpix, fixip=False, bstar=False, juststar=False):
        ...
        for i in range(ntel):
            ...
            try: bc = float(h[0].header['BC' + str(i+1)])
            except: bc = barycorr(midflux[i], ra[i], dec[i], pmra=pmra[i], pmdec=pmdec[i], parallax=parallax[i], rv=rv[i])
            zbc = np.append(zbc,bc/self.c)
...
def globgrind(globobs, bstar=False, returnfile=False, printit=False,plot=False,redo=False):
    ...
    for i, ffile in enumerate(files):
        ...
        ofile = os.path.splitext(ffile)[0] + '.chrec2.npy'
        ofarr.append(ofile)
...
#        ofarr = globgrind(datadir + '/*HR*.proc.fits', bstar=True, returnfile=False, printit=True, plot=False)
        ofarr = globgrind(datadir + '/*HD97601*.proc.fits', bstar=False, returnfile=False, printit=True, plot=False)
